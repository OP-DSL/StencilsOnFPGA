...
int bounds_2_l, int bounds_2_u) {
  ...
  if (n_0 < bounds_0_u && n_1 < bounds_1_u && n_2 < bounds_2_u) {
    const ACC<float> u(dims_poisson_kernel_error[0][0], dims_poisson_kernel_error[0][1], u_p + n_0 + n_1 * dims_poisson_kernel_error[0][0] + n_2 * dims_poisson_kernel_error[0][0] * dims_poisson_kernel_error[0][1]);
    const ACC<float> ref(dims_poisson_kernel_error[1][0], dims_poisson_kernel_error[1][1], ref_p + n_0 + n_1 * dims_poisson_kernel_error[1][0] + n_2 * dims_poisson_kernel_error[1][0] * dims_poisson_kernel_error[1][1]);
  ...
  *err = *err + (u(0,0)-ref(0,0))*(u(0,0)-ref(0,0));
  ...
  for (int d=0; d<1; d++)
    ...
    ops_reduction_cuda<OPS_INC>(&err_p[d+n_0*1],err[d],blockDim.y*blockDim.z, blockDim.x, 0);
  #elif defined(OPS_BATCHED) && OPS_BATCHED==2
    ops_reduction_cuda<OPS_INC>(&err_p[d+n_2*1],err[d],blockDim.x*blockDim.y, 1, 0);
  #elif defined(OPS_BATCHED)
  #error CUDA Reductions only implemented for OPS_BATCHED=0 or 2
  #else
    ops_reduction_cuda<OPS_INC>(&err_p[d+(blockIdx.x + blockIdx.y*gridDim.x)*1],err[d]);
  #endif

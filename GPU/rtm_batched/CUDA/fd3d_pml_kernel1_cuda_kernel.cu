...
//
__constant__ int dims_fd3d_pml_kernel1[12][4];
static int dims_fd3d_pml_kernel1_h[12][4] = {0};

// user function

__global__ void ops_fd3d_pml_kernel1(
    ...
    float *__restrict mu_p, float *__restrict yy_p, float *__restrict dyy_p,
  ...
  if (n_0 < bounds_0_u && n_1 < bounds_1_u && n_2 < bounds_2_u &&
      n_3 < bounds_3_u) {
    const ACC<float> rho(
        dims_fd3d_pml_kernel1[7][0], dims_fd3d_pml_kernel1[7][1],
        dims_fd3d_pml_kernel1[7][2],
        rho_p + n_0 + n_1 * dims_fd3d_pml_kernel1[7][0] +
            n_2 * dims_fd3d_pml_kernel1[7][0] * dims_fd3d_pml_kernel1[7][1] +
            n_3 * dims_fd3d_pml_kernel1[7][0] * dims_fd3d_pml_kernel1[7][1] *
                dims_fd3d_pml_kernel1[7][2]);
    const ACC<float> mu(
        dims_fd3d_pml_kernel1[8][0], dims_fd3d_pml_kernel1[8][1],
        dims_fd3d_pml_kernel1[8][2],
        mu_p + n_0 + n_1 * dims_fd3d_pml_kernel1[8][0] +
            n_2 * dims_fd3d_pml_kernel1[8][0] * dims_fd3d_pml_kernel1[8][1] +
            n_3 * dims_fd3d_pml_kernel1[8][0] * dims_fd3d_pml_kernel1[8][1] *
                dims_fd3d_pml_kernel1[8][2]);
#ifdef OPS_SOA
    const ACC<float> yy(
        6, dims_fd3d_pml_kernel1[9][0], dims_fd3d_pml_kernel1[9][1],
        dims_fd3d_pml_kernel1[9][2],
        yy_p + n_0 + n_1 * dims_fd3d_pml_kernel1[9][0] +
            n_2 * dims_fd3d_pml_kernel1[9][0] * dims_fd3d_pml_kernel1[9][1] +
            n_3 * dims_fd3d_pml_kernel1[9][0] * dims_fd3d_pml_kernel1[9][1] *
                dims_fd3d_pml_kernel1[9][2]);
#else
    const ACC<float> yy(6, dims_fd3d_pml_kernel1[9][0],
                        dims_fd3d_pml_kernel1[9][1],
                        dims_fd3d_pml_kernel1[9][2],
                        yy_p + 6 * (n_0 + n_1 * dims_fd3d_pml_kernel1[9][0] +
                                    n_2 * dims_fd3d_pml_kernel1[9][0] *
                                        dims_fd3d_pml_kernel1[9][1] +
                                    n_3 * dims_fd3d_pml_kernel1[9][0] *
                                        dims_fd3d_pml_kernel1[9][1] *
                                        dims_fd3d_pml_kernel1[9][2]));
#endif
#ifdef OPS_SOA
    ACC<float> dyy(
        6, dims_fd3d_pml_kernel1[10][0], dims_fd3d_pml_kernel1[10][1],
        dims_fd3d_pml_kernel1[10][2],
        dyy_p + n_0 + n_1 * dims_fd3d_pml_kernel1[10][0] +
            n_2 * dims_fd3d_pml_kernel1[10][0] * dims_fd3d_pml_kernel1[10][1] +
            n_3 * dims_fd3d_pml_kernel1[10][0] * dims_fd3d_pml_kernel1[10][1] *
                dims_fd3d_pml_kernel1[10][2]);
#else
    ACC<float> dyy(6, dims_fd3d_pml_kernel1[10][0],
                   dims_fd3d_pml_kernel1[10][1], dims_fd3d_pml_kernel1[10][2],
                   dyy_p + 6 * (n_0 + n_1 * dims_fd3d_pml_kernel1[10][0] +
                                n_2 * dims_fd3d_pml_kernel1[10][0] *
                                    dims_fd3d_pml_kernel1[10][1] +
                                n_3 * dims_fd3d_pml_kernel1[10][0] *
                                    dims_fd3d_pml_kernel1[10][1] *
                                    dims_fd3d_pml_kernel1[10][2]));
#endif
#ifdef OPS_SOA
    ACC<float> sum(
        6, dims_fd3d_pml_kernel1[11][0], dims_fd3d_pml_kernel1[11][1],
        dims_fd3d_pml_kernel1[11][2],
        sum_p + n_0 + n_1 * dims_fd3d_pml_kernel1[11][0] +
            n_2 * dims_fd3d_pml_kernel1[11][0] * dims_fd3d_pml_kernel1[11][1] +
            n_3 * dims_fd3d_pml_kernel1[11][0] * dims_fd3d_pml_kernel1[11][1] *
                dims_fd3d_pml_kernel1[11][2]);
#else
    ACC<float> sum(6, dims_fd3d_pml_kernel1[11][0],
                   dims_fd3d_pml_kernel1[11][1], dims_fd3d_pml_kernel1[11][2],
                   sum_p + 6 * (n_0 + n_1 * dims_fd3d_pml_kernel1[11][0] +
                                n_2 * dims_fd3d_pml_kernel1[11][0] *
                                    dims_fd3d_pml_kernel1[11][1] +
                                n_3 * dims_fd3d_pml_kernel1[11][0] *
                                    dims_fd3d_pml_kernel1[11][1] *
                                    dims_fd3d_pml_kernel1[11][2]));
#endif
...
// host stub function
#ifndef OPS_LAZY
void ops_par_loop_fd3d_pml_kernel1(char const *name, ops_block block, int dim,
...
#endif
#else
void ops_par_loop_fd3d_pml_kernel1_execute(const char *name, ops_block block,
  ...
  if (OPS_instance::getOPSInstance()->OPS_diags > 1) {
    ops_timing_realloc(1, "fd3d_pml_kernel1");
    OPS_instance::getOPSInstance()->OPS_kernels[1].count++;
    ops_timers_core(&__c2, &__t2);
  }

#ifdef OPS_DEBUG
  ops_register_args(args, "fd3d_pml_kernel1");
#endif
...
#ifdef OPS_MPI
  sub_dat_list sd = OPS_sub_dat_list[args[11].dat->index];
...
#endif
  if (args[7].dat->size[0] != dims_fd3d_pml_kernel1_h[7][0] ||
      args[7].dat->size[1] != dims_fd3d_pml_kernel1_h[7][1] ||
      args[7].dat->size[2] != dims_fd3d_pml_kernel1_h[7][2] ||
      args[7].dat->size[3] != dims_fd3d_pml_kernel1_h[7][3] ||
      args[8].dat->size[0] != dims_fd3d_pml_kernel1_h[8][0] ||
      args[8].dat->size[1] != dims_fd3d_pml_kernel1_h[8][1] ||
      args[8].dat->size[2] != dims_fd3d_pml_kernel1_h[8][2] ||
      args[8].dat->size[3] != dims_fd3d_pml_kernel1_h[8][3] ||
      args[9].dat->size[0] != dims_fd3d_pml_kernel1_h[9][0] ||
      args[9].dat->size[1] != dims_fd3d_pml_kernel1_h[9][1] ||
      args[9].dat->size[2] != dims_fd3d_pml_kernel1_h[9][2] ||
      args[9].dat->size[3] != dims_fd3d_pml_kernel1_h[9][3] ||
      args[10].dat->size[0] != dims_fd3d_pml_kernel1_h[10][0] ||
      args[10].dat->size[1] != dims_fd3d_pml_kernel1_h[10][1] ||
      args[10].dat->size[2] != dims_fd3d_pml_kernel1_h[10][2] ||
      args[10].dat->size[3] != dims_fd3d_pml_kernel1_h[10][3] ||
      args[11].dat->size[0] != dims_fd3d_pml_kernel1_h[11][0] ||
      args[11].dat->size[1] != dims_fd3d_pml_kernel1_h[11][1] ||
      args[11].dat->size[2] != dims_fd3d_pml_kernel1_h[11][2] ||
      args[11].dat->size[3] != dims_fd3d_pml_kernel1_h[11][3]) {
    dims_fd3d_pml_kernel1_h[7][0] = args[7].dat->size[0];
    dims_fd3d_pml_kernel1_h[7][1] = args[7].dat->size[1];
    dims_fd3d_pml_kernel1_h[7][2] = args[7].dat->size[2];
    dims_fd3d_pml_kernel1_h[7][3] = args[7].dat->size[3];
    dims_fd3d_pml_kernel1_h[8][0] = args[8].dat->size[0];
    dims_fd3d_pml_kernel1_h[8][1] = args[8].dat->size[1];
    dims_fd3d_pml_kernel1_h[8][2] = args[8].dat->size[2];
    dims_fd3d_pml_kernel1_h[8][3] = args[8].dat->size[3];
    dims_fd3d_pml_kernel1_h[9][0] = args[9].dat->size[0];
    dims_fd3d_pml_kernel1_h[9][1] = args[9].dat->size[1];
    dims_fd3d_pml_kernel1_h[9][2] = args[9].dat->size[2];
    dims_fd3d_pml_kernel1_h[9][3] = args[9].dat->size[3];
    dims_fd3d_pml_kernel1_h[10][0] = args[10].dat->size[0];
    dims_fd3d_pml_kernel1_h[10][1] = args[10].dat->size[1];
    dims_fd3d_pml_kernel1_h[10][2] = args[10].dat->size[2];
    dims_fd3d_pml_kernel1_h[10][3] = args[10].dat->size[3];
    dims_fd3d_pml_kernel1_h[11][0] = args[11].dat->size[0];
    dims_fd3d_pml_kernel1_h[11][1] = args[11].dat->size[1];
    dims_fd3d_pml_kernel1_h[11][2] = args[11].dat->size[2];
    dims_fd3d_pml_kernel1_h[11][3] = args[11].dat->size[3];
    cutilSafeCall(cudaMemcpyToSymbol(dims_fd3d_pml_kernel1,
                                     dims_fd3d_pml_kernel1_h,
                                     sizeof(dims_fd3d_pml_kernel1)));
  }
  ...
  if (x_size > 0 && y_size > 0 && z_size > 0)
    ops_fd3d_pml_kernel1<<<grid, tblock>>>(
        *dispx, *dispy, *dispz, *dt, *scale1, *scale2, rho_p, mu_p, yy_p, dyy_p,
        sum_p, blockidx_start,
#ifdef OPS_MPI
...
void ops_par_loop_fd3d_pml_kernel1(char const *name, ops_block block, int dim,
  ...
  desc->function = ops_par_loop_fd3d_pml_kernel1_execute;
  if (OPS_instance::getOPSInstance()->OPS_diags > 1) {
    ops_timing_realloc(1, "fd3d_pml_kernel1");
  }

...
#endif
#else
void ops_par_loop_final_update_kernel_execute(const char *name, ops_block block, int blockidx_start, int blockidx_end, int dim, int *range, int nargs, ops_arg* args) {
  #ifdef OPS_BATCHED
  ...
  float * __restrict__ k1_p = (float *)(args[5].data + args[5].dat->base_offset + blockidx_start * args[5].dat->batch_offset);

  float * __restrict__ k2_p = (float *)(args[6].data + args[6].dat->base_offset + blockidx_start * args[6].dat->batch_offset);

  float * __restrict__ k3_p = (float *)(args[7].data + args[7].dat->base_offset + blockidx_start * args[7].dat->batch_offset);

  float * __restrict__ k4_p = (float *)(args[8].data + args[8].dat->base_offset + blockidx_start * args[8].dat->batch_offset);

  float * __restrict__ yy_p = (float *)(args[9].data + args[9].dat->base_offset + blockidx_start * args[9].dat->batch_offset);
    ...
    for ( int n_2=bounds_2_l; n_2<bounds_2_u; n_2++ ){
      for ( int n_1=bounds_1_l; n_1<bounds_1_u; n_1++ ){
        ...
        for ( int n_0=bounds_0_l; n_0<bounds_0_u; n_0++ ){
          ...
          #endif
          #ifdef OPS_SOA
          const ACC<float> k1(6, xdim5, ydim5, zdim5, k1_p + n_0 + n_1 * xdim5 + n_2 * xdim5 * ydim5 + n_3 * xdim5 * ydim5 * zdim5);
          #else
          const ACC<float> k1(6, xdim5, ydim5, zdim5, k1_p + 6*(n_0 + n_1 * xdim5 + n_2 * xdim5 * ydim5 + n_3 * xdim5 * ydim5 * zdim5));
          #endif
          #ifdef OPS_SOA
          const ACC<float> k2(6, xdim6, ydim6, zdim6, k2_p + n_0 + n_1 * xdim6 + n_2 * xdim6 * ydim6 + n_3 * xdim6 * ydim6 * zdim6);
          #else
          const ACC<float> k2(6, xdim6, ydim6, zdim6, k2_p + 6*(n_0 + n_1 * xdim6 + n_2 * xdim6 * ydim6 + n_3 * xdim6 * ydim6 * zdim6));
          #endif
          #ifdef OPS_SOA
          const ACC<float> k3(6, xdim7, ydim7, zdim7, k3_p + n_0 + n_1 * xdim7 + n_2 * xdim7 * ydim7 + n_3 * xdim7 * ydim7 * zdim7);
          #else
          const ACC<float> k3(6, xdim7, ydim7, zdim7, k3_p + 6*(n_0 + n_1 * xdim7 + n_2 * xdim7 * ydim7 + n_3 * xdim7 * ydim7 * zdim7));
          #endif
          #ifdef OPS_SOA
          ACC<float> k4(6, xdim8, ydim8, zdim8, k4_p + n_0 + n_1 * xdim8 + n_2 * xdim8 * ydim8 + n_3 * xdim8 * ydim8 * zdim8);
          #else
          ACC<float> k4(6, xdim8, ydim8, zdim8, k4_p + 6*(n_0 + n_1 * xdim8 + n_2 * xdim8 * ydim8 + n_3 * xdim8 * ydim8 * zdim8));
          #endif
          #ifdef OPS_SOA
          ACC<float> yy(6, xdim9, ydim9, zdim9, yy_p + n_0 + n_1 * xdim9 + n_2 * xdim9 * ydim9 + n_3 * xdim9 * ydim9 * zdim9);
          #else
          ACC<float> yy(6, xdim9, ydim9, zdim9, yy_p + 6*(n_0 + n_1 * xdim9 + n_2 * xdim9 * ydim9 + n_3 * xdim9 * ydim9 * zdim9));
          #endif

...
void ops_par_loop_poisson_kernel_error_execute(const char *name, ops_block block, int blockidx_start, int blockidx_end, int dim, int *range, int nargs, ops_arg* args) {
  ...
  #if defined(_OPENMP) && defined(OPS_BATCHED) && !defined(OPS_LAZY) && OPS_BATCHED==3
  #pragma omp parallel for
  #endif
  for ( int n_3=bounds_3_l; n_3<bounds_3_u; n_3++ ){
    #if OPS_BATCHED==3 || !defined(OPS_BATCHED)
    float p_a2_0 = p_a2[n_3*1+0];
    #endif
    ...
    for ( int n_2=bounds_2_l; n_2<bounds_2_u; n_2++ ){
      ...
      for ( int n_1=bounds_1_l; n_1<bounds_1_u; n_1++ ){
        ...
        for ( int n_0=bounds_0_l; n_0<bounds_0_u; n_0++ ){
          ...
          #endif
          const ACC<float> u(xdim0, ydim0, zdim0, u_p + n_0 + n_1 * xdim0 + n_2 * xdim0 * ydim0 + n_3 * xdim0 * ydim0 * zdim0);
          const ACC<float> ref(xdim1, ydim1, zdim1, ref_p + n_0 + n_1 * xdim1 + n_2 * xdim1 * ydim1 + n_3 * xdim1 * ydim1 * zdim1);
          float err[1];
          err[0] = ZERO_float;

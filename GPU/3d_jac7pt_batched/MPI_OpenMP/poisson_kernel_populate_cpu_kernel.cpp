...
void ops_par_loop_poisson_kernel_populate_execute(const char *name, ops_block block, int blockidx_start, int blockidx_end, int dim, int *range, int nargs, ops_arg* args) {
  ...
  const int zdim2 = args[2].dat->size[2];
    ...
    for ( int n_2=bounds_2_l; n_2<bounds_2_u; n_2++ ){
      for ( int n_1=bounds_1_l; n_1<bounds_1_u; n_1++ ){
        ...
        for ( int n_0=bounds_0_l; n_0<bounds_0_u; n_0++ ){
          ...
          #endif
          ACC<float> u(xdim1, ydim1, zdim1, u_p + n_0 + n_1 * xdim1 + n_2 * xdim1 * ydim1 + n_3 * xdim1 * ydim1 * zdim1);
          ACC<float> f(xdim2, ydim2, zdim2, f_p + n_0 + n_1 * xdim2 + n_2 * xdim2 * ydim2 + n_3 * xdim2 * ydim2 * zdim2);
          ACC<float> ref(xdim3, ydim3, zdim3, ref_p + n_0 + n_1 * xdim3 + n_2 * xdim3 * ydim3 + n_3 * xdim3 * ydim3 * zdim3);

...
void buildOpenCLKernels_poisson_kernel_stencil(int xdim0, int ydim0, int xdim1, int ydim1) {
  ...
  if(!isbuilt_poisson_kernel_stencil) {
    ...
    printf("Compiling poisson_kernel_stencil %d source -- start \n",OCL_FMA);
      ...
      if (pPath!=NULL)
        if(OCL_FMA)
          sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_poisson_kernel_stencil=%d  -Dydim0_poisson_kernel_stencil=%d  -Dxdim1_poisson_kernel_stencil=%d  -Dydim1_poisson_kernel_stencil=%d ", pPath, 32,xdim0,ydim0,xdim1,ydim1);
        else
          sprintf(buildOpts,"-cl-mad-enable -I%s/c/include -DOPS_WARPSIZE=%d  -Dxdim0_poisson_kernel_stencil=%d  -Dydim0_poisson_kernel_stencil=%d  -Dxdim1_poisson_kernel_stencil=%d  -Dydim1_poisson_kernel_stencil=%d ", pPath, 32,xdim0,ydim0,xdim1,ydim1);
      else {
...
void ops_par_loop_poisson_kernel_stencil(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(OPS_instance::getOPSInstance()->opencl_instance->OPS_opencl_core.kernel[3], 6, sizeof(cl_int), (void*) &z_size ));

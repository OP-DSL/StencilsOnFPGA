		...
		for(int i = 0; i < data_g.act_size_y; i++){
		  for(int j = 0; j < data_g.act_size_x; j++){
			if(i == 0 || j == 0 || i == data_g.act_size_x -1  || j==data_g.act_size_y-1){
			  ...
			  next[i*data_g.grid_size_x + j + offset] = current[(i-1)*data_g.grid_size_x + (j-1) + offset] * (-0.07) + \
					  	  	  	  	  	  	  	 current[(i)*data_g.grid_size_x + (j-1)+offset] * (-0.08) + \
												 current[(i+1)*data_g.grid_size_x + (j-1)+offset] * (-0.01) + \
												 current[(i-1)*data_g.grid_size_x + (j)+ offset] *   (-0.06) + \
												 current[(i)*data_g.grid_size_x + (j)+offset] *   (0.36)   + \
												 current[(i+1)*data_g.grid_size_x + (j)+offset]   * (-0.02) + \
												 current[(i-1)*data_g.grid_size_x + (j+1)+offset] * (-0.05) + \
												 current[(i)*data_g.grid_size_x + (j+1)+offset] * (-0.04) + \
												 current[(i+1)*data_g.grid_size_x + (j+1)+offset] * (-0.03) ;

...
template <int pidx>
static void derives_calc_ytep_k3( queue &q, struct data_G data_g, ac_int<12,true> n_iter){
		...
		for(ac_int<12,true> u_itr = 0; u_itr < n_iter; u_itr++){
			...
			for(unsigned int itr = 0; itr < gridsize; itr++) {
				...
				if(cond_wr ) {
					...
					pipeM::PipeAt<pidx+11>::write(yy_vec);
					pipeM::PipeAt<pidx+21>::write(yy_final_vec_write);
				}

...
static void derives_calc_ytep_k3( queue &q, struct data_G data_g){
    ...
    h.single_task<class derives_calc_ytep_k3>([=] () [[intel::kernel_args_restrict]]{
		...
		for(unsigned int itr = 0; itr < gridsize; itr++) {
			...
			if(cond_tmp1){
				s_4_4_8 = pipeM::PipeAt<2>::read(); // set
			}
			...
			if(cond_tmp1){
				yy_vec_tmp = pipeM::PipeAt<12>::read(); // set
			}
			...
			if(cond_tmp1){
				yy_final_vec_tmp = pipeM::PipeAt<22>::read(); // set
			}
			...
			for(int v = 0; v < 3; v++){
			  	...
			  	float sigma = s_4_4_4.data[1+v*8]/s_4_4_4.data[0+v*8]; 
			  	float sigmax=0.0;
			  	float sigmay=0.0;
			  	float sigmaz=0.0;
				...
				wr_vec.data[7+v*8]= k_7_2 - k_7_3;  //(pxz+pyz+pzz)*mu[OPS_ACC5(0,0,0)] - sigmaz*vz;
		  		...
		  		// calc Y temp - yyTmp_vec
		  		
		  		yyTmp_vec.data[2+8*v] = yy_vec.data[2+8*v] + wr_vec_dt.data[2+8*v];
		  		yyTmp_vec.data[5+8*v] = yy_vec.data[5+8*v] + wr_vec_dt.data[5+8*v];
		  	
		  		yyTmp_vec.data[3+8*v] = yy_vec.data[3+8*v] + wr_vec_dt.data[3+8*v];
		  		yyTmp_vec.data[6+8*v] = yy_vec.data[6+8*v] + wr_vec_dt.data[6+8*v];
		  	
		  		yyTmp_vec.data[4+8*v] = yy_vec.data[4+8*v] + wr_vec_dt.data[4+8*v];
		  		yyTmp_vec.data[7+8*v] = yy_vec.data[7+8*v] + wr_vec_dt.data[7+8*v];

		  		yyTmp_vec.data[0+8*v] = s_4_4_4.data[0+8*v];
		  		yyTmp_vec.data[1+8*v] = s_4_4_4.data[1+8*v];
			...
			if(cond_wr ) {
				pipeM::PipeAt<3>::write(update_j);
				pipeM::PipeAt<13>::write(yy_vec);
				pipeM::PipeAt<23>::write(yy_final_vec_write);
			}

...
// template <int VFACTOR>
// void stencil_write(queue &q, buffer<struct dPath16, 1> &out_buf, ac_int<12,true> nx, ac_int<12,true> ny, ac_int<12,true> nz,
...
template <int N, int n> struct loop {
  static void instantiate(queue &q, int nx, int ny, int nz, int batch, int n_iter){
    loop<N-1, n-1>::instantiate(q, nx, ny, nz, batch, n_iter);
    stencil_compute<N-1, n-1, 128, 8>(q, nx, ny, nz, batch, n_iter);
  }
};
...
struct loop<1, 1>{
  static void instantiate(queue &q, int nx, int ny, int nz, int batch, int n_iter){
    stencil_compute<0, 0, 128, 8>(q, nx, ny, nz, batch, n_iter);
  }
};
...
//************************************
void stencil_comp(queue &q, IntVector  &input, IntVector &output, int n_iter, int nx, int ny, int nz, int batch, int delay) {
  ...
  dpc_common::TimeInterval exe_time;
    ...
    // for(int itr = 0; itr < n_iter; itr++){
      ...
      // reading from memory
      event e1 = stencil_read_write<16>(q, in_buf, out_buf, nx, ny, nz, batch, n_iter, delay);
      PipeConvert_512_256<8>(q,nx, ny, nz, batch,n_iter);
      loop<UFACTOR, UFACTOR>::instantiate(q, nx, ny, nz, batch, n_iter);
      PipeConvert_256_512<UFACTOR, 8>(q,nx, ny, nz, batch, n_iter);
      //write back to memory
      // stencil_write<16>(q, out_buf, nx, ny, nz, batch, kernel_time);
      q.wait();

      double start0 = e1.get_profiling_info<info::event_profiling::command_start>();
      double end0 = e1.get_profiling_info<info::event_profiling::command_end>(); 
      kernel_time += (end0-start0)*1e-9;

      
      // // reading from memory
      // stencil_read_write<16>(q, out_buf, in_buf, nx, ny, nz, batch, delay, kernel_time);
      ...
      // //write back to memory
      // // stencil_write<16>(q, in_buf, nx, ny, nz, batch, kernel_time);
...
int main(int argc, char* argv[]) {
  ...
  in_vec.resize(nx/v_factor*ny*nz*batch + delay*2);
  in_vec_h.resize(nx*ny*nz*batch);
  out_sequential.resize(nx*ny*nz*batch);
  out_parallel.resize(nx/v_factor*ny*nz*batch + delay*2);
  ...
  try {
    ...
    stencil_comp(q, in_vec, out_parallel, 2*n_iter, nx, ny, nz, batch, delay);
      ...
      for(int j = 0; j < ny; j++){
        for(int i = 0; i < nx/v_factor; i++){
          for(int v = 0; v < v_factor; v++){
            int ind = bat*nx*ny*nz + k*nx*ny + j*nx + i*v_factor;
            // std::cout << "j,i: " << j  << " " << i << " " << in_vec_h.at(ind+v) << " " << in_vec.at(ind/v_factor).data[v] <<  std::endl;
            ...
            if(chk > 0.00001 && fabs(in_vec_h.at(ind+v)) > 0.00001){
              std::cout << "j,i: " << j  << " " << i << " " << in_vec_h.at(ind+v) << " " << in_vec.at((ind)/v_factor+delay).data[v] <<  std::endl;
              return -1;
            }
          }
        }

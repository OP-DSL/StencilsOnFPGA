static void derives_calc_ytep_k4( queue &q, struct data_G data_g){
    ...
    h.single_task<class derives_calc_ytep_k4>([=] () [[intel::kernel_args_restrict]]{
		...
		for(unsigned int itr = 0; itr < gridsize; itr++) {
			...
			if(cond_tmp1){
				s_4_4_8 = pipeS::PipeAt<3>::read();  // set
			}
			...
			if(cond_tmp1){
				 yy_vec_tmp = pipeS::PipeAt<13>::read();  // set
			}
			...
			if(cond_tmp1){
				yy_final_vec_tmp = pipeS::PipeAt<23>::read();  // set
			}
			...
			wr_vec.data[7]= k_7_2 - k_7_3;  //(pxz+pyz+pzz)*mu[OPS_ACC5(0,0,0)] - sigmaz*vz;
	  		...
	  		// YY final 
	  		yy_final_vec.data[2] += wr_vec_dt.data[2] * 0.1666666667f;
	  		yy_final_vec.data[5] += wr_vec_dt.data[5] * 0.1666666667f;

	  		yy_final_vec.data[3] += wr_vec_dt.data[3] * 0.1666666667f;
	  		yy_final_vec.data[6] += wr_vec_dt.data[6] * 0.1666666667f;

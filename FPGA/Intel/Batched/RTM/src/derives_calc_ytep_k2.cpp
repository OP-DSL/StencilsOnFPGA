static void derives_calc_ytep_k2( queue &q, struct data_G data_g){
    ...
    h.single_task<class derives_calc_ytep_k2>([=] () [[intel::kernel_args_restrict]]{
		...
		for(unsigned int itr = 0; itr < gridsize; itr++) {
			...
			if(cond_tmp1){
				s_4_4_8 = pipeS::PipeAt<0>::read(); // set
			}
			...
			if(cond_tmp1){
				yy_vec_tmp = pipeS::PipeAt<10>::read(); // set
			}
			...
			if(cond_tmp1){
				yy_final_vec_tmp = pipeS::PipeAt<20>::read(); // set
			}
	...
	// X ARM
			...
			if(cond_wr ) {
				...
				pipeS::PipeAt<11>::write(update_j);
				pipeS::PipeAt<21>::write(update_j);

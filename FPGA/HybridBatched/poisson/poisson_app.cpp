...
int main(int argc, char **argv)
{
  ...
  data_g.grid_size_x = data_g.act_size_x * 8; //(data_g.act_size_x % 8) != 0 ? (data_g.act_size_x/8 +1) * 8 : data_g.act_size_x;
  ...
  //OPENCL HOST CODE AREA START
	...
	//Launch the Kernel
	OCL_CHECK(err, err = q.enqueueTask(krnl_slr0, NULL, &event));
...
//  // golden stencil computation on host
  ...
  float bandwidth = (8*data_g.logical_size_x * data_g.logical_size_y * sizeof(float) * 4.0 * n_iter * data_g.batch*60)/(k_time * 1000 * 1000 * 1000);
